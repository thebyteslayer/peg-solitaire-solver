...
func startStateAt(empty int) State { var s State; for i:=0;i<nCells;i++{s.set(i)}; s.clr(empty); return s }
func clsSingle(b int) invClass { var s State; s.set(b); return classOf(s) }
func solveWithin(d time.Duration) bool {
	done := make(chan struct{})
	go func(){ select { case <-done: case <-time.After(d): atomic.StoreInt32(&stopFlag,1) } }()
	ok := defaultSearch(); close(done); return ok
}
func rebuildSymmetry(sp boardSpec, g int) {
	centerID, goalPegID = g, g
	perms = perms[:0]
	for _, tf := range sp.candidateTransforms() {
		p := make([]int, nCells); ok := true
		for i:=0;i<nCells;i++{ tr,tc:=tf(id2r[i],id2c[i]); if !inBox(tr,tc)||rc2id[tr][tc]<0 {ok=false;break}; p[i]=rc2id[tr][tc] }
		if ok && p[g]==g { perms = append(perms,p) }
	}
	nTransforms=len(perms); buildCanonTables()
}
...
func TestDiamondFrom13(t *testing.T) {
	...
	var cs []cand
	for g:=0; g<nCells; g++ {
		if g==a || clsSingle(g)!=want { continue }
